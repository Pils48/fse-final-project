...
if __name__ == '__main__':
    ...
    CMD_PARSER.add_argument('-t', '--threshold', metavar='threshold', type=float, default=0.1,
                            help='voxels with confidence lower than the threshold\
                            are not displayed')
    CMD_PARSER.add_argument('-i', '--index', metavar='index', type=int, default=1,
                            help='the index of objects in the inputfile that should be\
                            rendered (one based)')
    ...
    CMD_PARSER.add_argument('-cm', '--colormap', action="store_true",
                            help='whether to use a colormap to represent voxel occupancy,\
                            or to use a uniform color')
